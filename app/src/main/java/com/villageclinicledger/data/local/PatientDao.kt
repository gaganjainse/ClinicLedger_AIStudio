...
interface PatientDao {
     ...
     */
    @Query("SELECT DISTINCT p.* FROM patients p LEFT JOIN aliases a ON p.id = a.patient_id LEFT JOIN family_groups fg ON p.family_group_id = fg.id WHERE p.name LIKE '%' || :searchQuery || '%' OR p.phone LIKE '%' || :searchQuery || '%' OR p.village_id IN (SELECT id FROM villages WHERE name LIKE '%' || :searchQuery || '%') OR a.alias LIKE '%' || :searchQuery || '%' OR fg.name LIKE '%' || :searchQuery || '%' ORDER BY p.name ASC")
    ...
    @Query("UPDATE patients SET current_balance = :balance WHERE id = :patientId")
    suspend fun setPatientBalance(patientId: Long, balance: Double)
